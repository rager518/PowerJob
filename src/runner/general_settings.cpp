...
json::JsonObject GeneralSettings::to_json()
{
	...
	result.SetNamedValue(L"startup", json::value(isStartupEnabled));// start on boot
	...
	result.SetNamedValue(L"context_menu", json::value(contextMenu));
...
json::JsonObject load_general_settings()
{
	...
	context_menu = loaded.GetNamedBoolean(L"contextMenu", true);
...
void apply_general_settings(const json::JsonObject& general_configs, bool save)
{
	...
	context_menu = general_configs.GetNamedBoolean(L"contextmenu", true);
	...
	{
		// close autorun
		const bool startup = false;// general_configs.GetNamedBoolean(L"startup");
	...
	if (json::has(general_configs, L"enabled"))
	{
		for (const auto& enabled_element : general_configs.GetNamedObject(L"enabled"))
		{
			...
			if (gpo_rule == PowerJob_gpo::gpo_rule_configured_enabled || gpo_rule == PowerJob_gpo::gpo_rule_configured_disabled)
			{
				// Apply the GPO Rule.
				target_enabled = gpo_rule == PowerJob_gpo::gpo_rule_configured_enabled;
			}
	...
	if (save)
	{
		...
		// match contextmenu app
		if (save_settings.contextMenu) {
			addRightClickMenuItemEmpty();
		}
		else {
			removeRightClickMenuItemEmpty();
		}
	}
}
...
void start_enabled_powertoys()
{
	...
	std::unordered_map<std::wstring, PowerJob_gpo::gpo_rule_configured_t> PowerJob_gpo_configuration;
	// Take into account default values supplied by modules themselves and gpo configurations
	for (auto& [name, powertoy] : modules())
	{
		auto gpo_rule = powertoy->gpo_policy_enabled_configuration();
		PowerJob_gpo_configuration[name] = gpo_rule;
		if (gpo_rule == PowerJob_gpo::gpo_rule_configured_unavailable)
		{
			Logger::warn(L"start_enabled_powertoys: couldn't read the gpo rule for Powertoy {}", name);
		}
		if (gpo_rule == PowerJob_gpo::gpo_rule_configured_wrong_value)
		{
	...
	try
	{
		...
		if (general_settings.HasKey(L"enabled"))
		{
			...
			for (const auto& disabled_element : enabled)
			{
				...
				if (PowerJob_gpo_configuration.find(disable_module_name) != PowerJob_gpo_configuration.end()
					&& (PowerJob_gpo_configuration[disable_module_name] == PowerJob_gpo::gpo_rule_configured_enabled || PowerJob_gpo_configuration[disable_module_name] == PowerJob_gpo::gpo_rule_configured_disabled))
				{
	...
	for (auto& [name, powertoy] : modules())
	{
		...
		auto gpo_rule = PowerJob_gpo_configuration.find(name) != PowerJob_gpo_configuration.end() ? PowerJob_gpo_configuration[name] : PowerJob_gpo::gpo_rule_configured_not_configured;

		if (gpo_rule == PowerJob_gpo::gpo_rule_configured_enabled || gpo_rule == PowerJob_gpo::gpo_rule_configured_disabled)
		{
			// Apply the GPO Rule.
			should_powertoy_be_enabled = gpo_rule == PowerJob_gpo::gpo_rule_configured_enabled;
